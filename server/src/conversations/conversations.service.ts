...
export class ConversationsService {
  ...
  async updateGroupConversation(
    ...
    const updatedGroup = await this.conversationModel.findOneAndUpdate(
      { _id: convoId, groupOwner: groupOwnerId },
      { $set: updateFields },
      { new: true },
    )
  ...
  async leaveGroupConversation(convoId: string, userId: string) {
    const leaveConversation = await this.conversationModel.findOneAndUpdate(
      {
        _id: convoId,
  ...
  async removeGroupParticipants(
    ...
    const findGroup = await this.conversationModel.findOneAndUpdate(
      {
        _id: convoId,
        groupOwner: groupOwnerId,
      },
