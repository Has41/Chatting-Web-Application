...
} from "@shared/constants/constantValues";
...
const validateFile = async (file: File | string): Promise<ValidationResult> => {
  ...
  const rules: FileValidationRule =
    FILE_VALIDATION_RULES[fileType as keyof typeof FILE_VALIDATION_RULES] ||
    FILE_VALIDATION_RULES.default;

  const errorMessage = rules.error || rules.sizeError || "File size too large";
  const sizeValidation = validateSize(
    file as File,
    rules.maxSize,
    `${errorMessage} (File size: ${(file as File).size})`,
  );
